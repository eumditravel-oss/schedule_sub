...
test.describe('Task Hierarchy, Multi-Assignees, Auto Progress & Compact Gantt Rows', () => {
  ...
  test('E2E Full Flow: Hierarchy, Compact UI, Translation Protection & Delete Modal', async ({ page }) => {
    ...
    await page.fill('[data-testid="project-start-date-input"]', '2026-08-01');
    await page.fill('[data-testid="project-end-date-input"]', '2026-08-31');
    ...
    await page.waitForSelector('[data-testid="desktop-schedule-toolbar"]');

    // 4. Add Task Group (공정 대분류 추가)
    ...
    // 6. Verify Task Row & Compact Height
    ...
    // 7. Verify UI Element Text Removals (Count = 0)
    ...
    // 8. Verify Task Group Delete Modal with existing tasks
