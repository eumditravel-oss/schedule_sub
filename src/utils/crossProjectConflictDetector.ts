// src/utils/crossProjectConflictDetector.ts
  ...
  for (const worker of workers || []) {
    ...
    while (cur <= endObj) {
      ...
      if (dayStatus.is_working_day) {
        const activeDailyTasks = workerTasks.filter((t) => t.start_date && t.end_date && t.start_date <= dateStr && t.end_date >= dateStr);
...
function buildGroupObject(
  ...
  activeProjectMap: Map<string, Project>,
