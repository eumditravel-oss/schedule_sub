...
import { isProjectBoardRoute, isSchedulerRoute } from '../../utils/navigationPolicy';
...
export function AppShell({ children, worker }: AppShellProps) {
  ...
  const dashboardLabel = manager ? 'Team Dashboard' : 'Dashboard';
  const landing = resolveLandingRoute(resolvedWorker);
  const projectBoardActive = isProjectBoardRoute(location.pathname);
  const schedulerActive = isSchedulerRoute(location.pathname);
  const worklogActive = location.pathname === '/worklog' || location.pathname.startsWith('/worklog/');
  const reportActive = location.pathname === '/print' || location.pathname.startsWith('/print/');
  const managerActive = location.pathname === '/manager' || location.pathname.startsWith('/manager/');

  return <div className="min-h-screen bg-slate-100 text-slate-900">
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex min-h-16 max-w-[1600px] items-center gap-3 px-4 sm:px-6">
        <Link to={landing} className="flex min-w-0 items-center gap-2" aria-label="CON-COST home">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Main menu">
          {!viewer && <NavItem to="/dashboard" icon={LayoutDashboard} label={dashboardLabel} active={location.pathname === '/dashboard'} />}
          <NavItem to="/project-board" icon={FolderKanban} label="프로젝트" active={projectBoardActive} />
          {!viewer && <NavItem to="/worklog/today" icon={ClipboardCheck} label="업무일지" active={worklogActive} />}
          <NavItem to="/projects" icon={CalendarRange} label="스케줄러" active={schedulerActive} />
          <NavItem to="/print/projects/year-a4" icon={History} label="보고서·이력" active={reportActive} />
          {manager && <NavItem to="/manager/operations" icon={BarChart3} label="운영 현황" active={managerActive} />}
        </nav>
      ...
      <nav className="mx-auto flex max-w-[1600px] gap-1 overflow-x-auto border-t border-slate-100 px-4 py-2 lg:hidden sm:px-6" aria-label="Mobile menu">
        {!viewer && <NavItem to="/dashboard" icon={LayoutDashboard} label={dashboardLabel} active={location.pathname === '/dashboard'} />}
        <NavItem to="/project-board" icon={FolderKanban} label="프로젝트" active={projectBoardActive} />
        {!viewer && <NavItem to="/worklog/today" icon={ClipboardCheck} label="업무일지" active={worklogActive} />}
        <NavItem to="/projects" icon={CalendarRange} label="스케줄러" active={schedulerActive} />
