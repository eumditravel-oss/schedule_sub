          ...
          {worklogs.slice(0, 10).map((worklog) => (
            <li key={worklog.id}>
              <button type="button" onClick={() => onOpen(worklog)} className="flex w-full items-center gap-3 px-1 py-3 text-left hover:bg-slate-50">
                <div className="min-w-0 flex-1"><p className="text-sm font-bold text-slate-900">{worklog.local_work_date}</p><p className="mt-0.5 text-xs text-slate-500">{worklog.status} · {t('revision')} {worklog.current_revision_number || 0}</p></div>
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/40 p-3 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="worklog-history-title">
      <div className="max-h-[85vh] w-full max-w-2xl overflow-auto rounded-2xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4"><div><h2 id="worklog-history-title" className="text-lg font-extrabold text-slate-900">{worklog.local_work_date}</h2><p className="text-sm text-slate-500">{worklog.status}</p></div><button type="button" onClick={onClose} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-bold text-slate-700">{t('back')}</button></div>
        <section className="mt-5"><h3 className="font-extrabold text-slate-900">{t('revisionHistory')}</h3><ol className="mt-3 space-y-2">{(worklog.revisions || []).map((revision: any) => <li key={revision.id} className="rounded-lg border border-slate-200 p-3 text-sm"><p className="font-bold text-slate-900">{t('revision')} {revision.revision_number} · {revision.phase}</p><dl className="mt-2 grid gap-1 text-xs text-slate-600"><div><dt className="inline font-bold">{t('changedAt')}: </dt><dd className="inline">{revision.created_at || '-'}</dd></div><div><dt className="inline font-bold">{t('changeType')}: </dt><dd className="inline">{revision.change_type || '-'}</dd></div>{revision.reason && <div><dt className="inline font-bold">{t('reason')}: </dt><dd className="inline">{revision.reason}</dd></div>}</dl></li>)}</ol></section>
        <section className="mt-5"><h3 className="font-extrabold text-slate-900">{t('actual')}</h3><ul className="mt-3 space-y-2">{(worklog.entries || []).filter((entry: any) => entry.phase === 'EOD').map((entry: any) => <li key={entry.id} className="rounded-lg bg-slate-50 p-3 text-sm"><p className="font-bold text-slate-800">{entry.work_category}</p><p className="mt-1 text-xs text-slate-600">{Number(entry.actual_minutes || 0)}{t('minutes')} {entry.work_result ? `· ${entry.work_result}` : ''}</p></li>)}</ul></section>
      </div>
    </div>
  );
}
