    ...
    <section className="mx-auto max-w-[1500px] space-y-4 p-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
        <div className="space-y-4">
          ...
          <section data-testid="qa-stored-fact" className="rounded-xl border-2 border-slate-300 bg-white p-5 shadow-sm"><h2 className="mb-4 text-sm font-black">{t.stored}</h2><div className="grid gap-3 text-xs sm:grid-cols-2 lg:grid-cols-4"><StoredMetric label="Worklog ID" value={worklog?.id || '-'} /><StoredMetric label="Effective Revision" value={worklog ? `${worklog.effective_revision_number} · ${worklog.effective_revision_id}` : '-'} /><StoredMetric label="Stored status" value={worklog?.effective_status || 'NOT_CREATED'} /><StoredMetric label="Stored Actual" value={`${Number(worklog?.actual_recorded_minutes || 0)} min`} /><StoredMetric label="Stored Progress" value={effectiveTaskEntry?.progress_after == null ? '-' : `${effectiveTaskEntry.progress_after}%`} /><StoredMetric label="Stored Remaining" value={effectiveTaskEntry?.remaining_estimated_minutes == null ? '-' : `${effectiveTaskEntry.remaining_estimated_minutes} min`} /><StoredMetric label="Stored Capacity" value={`${Number(worklog?.capacity_minutes ?? capacity)} min`} /><StoredMetric label="Stored Gap" value={Number(worklog?.has_gap) === 1 ? worklog?.gap_reason_code : 'NONE'} /><StoredMetric label="Stored Overtime" value={Number(worklog?.overtime_candidate_minutes || 0) > 0 ? `${worklog.overtime_candidate_minutes} min · ${worklog.overtime_approval_status}` : 'NONE'} /><StoredMetric label="Last Updated" value={effectiveRevision?.created_at || '-'} /><StoredMetric label="Updated By" value={effectiveRevision?.created_by_employee_id || '-'} /><StoredMetric label="Change Type" value={worklog?.effective_change_type || '-'} /></div></section>
        </div>
        <div className="space-y-4">
