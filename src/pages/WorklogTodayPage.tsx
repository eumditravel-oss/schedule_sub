...
export function WorklogTodayPage({ initialView = 'TODAY' }: WorklogTodayPageProps) {
  ...
  const loadContext = useCallback(async (employeeId: string, date: string, view = snapshot(employeeId, date)) => {
    ...
    try {
      ...
      const nextMorningEntries = phaseEntries(nextContext.worklog?.current_morning_revision_id ? nextContext.worklog : null, 'MORNING', tasks);
      const rawEodEntries = nextContext.worklog?.current_eod_revision_id
        ? phaseEntries(nextContext.worklog, 'EOD', tasks)
        : eodEntriesFromMorning(nextContext.worklog, tasks);
      const plannedByTask = new Map(nextMorningEntries.filter((entry: WorklogEntryDraft) => entry.taskId).map((entry: WorklogEntryDraft) => [entry.taskId, Number(entry.plannedMinutes || 0)]));
      const nextEodEntries = rawEodEntries.map((entry: WorklogEntryDraft) => ({ ...entry, plannedMinutes: Number(entry.plannedMinutes || plannedByTask.get(entry.taskId || '') || 0) }));
      setMorningEntries(nextMorningEntries);
      setEodEntries(nextEodEntries);
  ...
  const openSchedule = (entry: WorklogEntryDraft) => {
    if (!entry.projectId || !entry.taskId) return;
    navigate(`/projects/${encodeURIComponent(entry.projectId)}?taskId=${encodeURIComponent(entry.taskId)}`);
  };
    ...
    <main className="min-h-screen bg-slate-50 pb-24" data-testid="employee-worklog-page">
      ...
      <div className="mx-auto max-w-6xl space-y-4 px-4 py-5 sm:px-6">
        ...
        {view === 'HISTORY' ? <RecentWorklogs language={language} worklogs={history} loading={historyLoading} onOpen={openHistory} /> : <>
          ...
          <section className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-base font-extrabold text-slate-900">{activeMode === 'MORNING' ? t('plan') : t('workResult')}</h2><p className="mt-1 text-xs text-slate-500">{t('official')}</p></div>{currentModeCanSubmit && <button type="button" onClick={addOtherWork} className="inline-flex h-9 items-center gap-1 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-bold text-blue-700 hover:bg-blue-100"><Plus className="h-4 w-4" />{t('addWork')}</button>}</div>{entries.length === 0 && <div className="rounded-xl border border-dashed border-slate-300 bg-white p-6 text-sm text-slate-600">{t('noTasks')}</div>}{entries.map((entry) => <WorklogEntryCard key={entry.id} entry={entry} mode={activeMode} language={language} readOnly={modeReadOnly} currentProgress={Number(taskActuals[entry.taskId || '']?.current_progress || taskActuals[entry.taskId || '']?.currentProgress || 0)} onChange={(updated) => replaceEntry(activeMode, updated)} onRemove={() => removeEntry(activeMode, entry.id)} onOpenSchedule={openSchedule} canRemove={!entry.taskId || entries.length > 1} taskOptions={eligibleTasks} fullDayMinutes={capacity} />)}</section>
