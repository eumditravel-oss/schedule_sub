      ...
      <main className="mx-auto max-w-[1700px] space-y-4 p-4 md:p-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><h2 className="flex items-center gap-2 text-sm font-black"><Play className="h-4 w-4 text-orange-500" />{lang === 'vi' ? 'Tính toán Shadow' : 'Shadow 재산정'}</h2><p className="mt-1 text-[10px] text-slate-500">Engine {shadow?.run?.engine_version || '3A.1.4'} · {shadow?.run?.input_fingerprint?.slice(0, 16) || 'no run'}</p></div>
