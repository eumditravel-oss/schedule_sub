      ...
      <main className="mx-auto max-w-[1700px] space-y-4 p-4 md:p-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><h2 className="flex items-center gap-2 text-sm font-black"><Play className="h-4 w-4 text-orange-500" />{lang === 'vi' ? 'Tính toán Shadow' : 'Shadow 재산정'}</h2><p className="mt-1 text-[10px] text-slate-500">Engine {shadow?.run?.engine_version || '3A.1.12'} · {shadow?.run?.input_fingerprint?.slice(0, 16) || 'no run'}</p></div>
          ...
          {shadow?.run && <div data-testid="shadow-run-summary" className="mt-3 grid gap-2 rounded-lg bg-slate-50 p-3 text-[10px] text-slate-600 md:grid-cols-3 xl:grid-cols-5"><span>Worklog: {shadow.run.source_worklog_id || 'MANUAL'}</span><span>Revision: {shadow.run.source_revision_id || '—'}</span><span>Engine: {shadow.run.engine_version}</span><span>{new Date(shadow.run.completed_at || shadow.run.planning_cutoff_utc).toLocaleString()}</span><span>Forecast V{shadow.run.based_on_forecast_version || '—'}</span><span>Shadow V{projectVersion?.shadow_version_number || '—'}</span><span>Confidence: {shadow.run.data_confidence}</span><span>Approval: {approvalRequired ? 'REQUIRED' : 'NOT REQUIRED'}</span><span>{shadow.run.affected_project_count} projects</span><span>{shadow.run.affected_task_count} tasks</span></div>}
        </section>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><h2 className="mb-3 text-sm font-black">Task Before / After Diff</h2><div className="overflow-x-auto"><table className="w-full min-w-[1320px] text-left text-[10px]"><thead className="bg-slate-50 text-slate-500"><tr><th className="p-2">{lang === 'vi' ? 'Dự án' : '프로젝트'}</th><th className="p-2">WBS / Task</th><th className="p-2">{lang === 'vi' ? 'Nhân viên' : '직원'}</th><th className="p-2">Official Start</th><th className="p-2">Shadow Start</th><th className="p-2">Official End</th><th className="p-2">Shadow End</th><th className="p-2">Δ</th><th className="p-2">{lang === 'vi' ? 'Nguyên nhân' : '원인'}</th><th className="p-2">Constraint</th><th className="p-2">Confidence</th><th className="p-2">{lang === 'vi' ? 'Cần phê duyệt' : '승인 필요'}</th></tr></thead><tbody>{impactedTasks.map((task) => <tr key={task.task_id} className="border-t border-slate-100"><td className="p-2">{task.project_name}</td><td className="p-2 font-bold">{task.task_sort_order}. {lang === 'vi' ? task.task_name_vi || task.task_name : task.task_name_ko || task.task_name}<div className="text-[9px] font-normal text-slate-400">{groupById.get(tasks.find((item) => item.id === task.task_id)?.task_group_id || '')?.group_name || ''}</div></td><td className="p-2">{task.employee_name || '—'}</td><td className="p-2">{fmtDate(task.official_forecast_start)}</td><td className="p-2 font-bold text-orange-700">{fmtDate(task.shadow_start)}</td><td className="p-2">{fmtDate(task.official_forecast_end)}</td><td className="p-2 font-bold text-orange-700">{fmtDate(task.shadow_end)}</td><td className="p-2">{Number(task.delta_end_workdays) > 0 ? '+' : ''}{task.delta_end_workdays}</td><td className="max-w-xs p-2 text-slate-500">{parseCodes(task.impact_reason_codes_json).join(', ') || '—'}</td><td className="p-2">{task.constraint_result || '—'}</td><td className="p-2"><Badge value={task.data_confidence} /></td><td className="p-2">{Number(task.approval_required) ? <Badge value="APPROVAL_REQUIRED" /> : '—'}</td></tr>)}</tbody></table></div></section>
      </main>
    </div>
  );
}
