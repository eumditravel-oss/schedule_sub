...
function ShadowGantt({ tasks, lang, holidays, overrides }: {
  ...
  const hatchDates = new Map<string, 'KR' | 'VN' | 'BOTH' | 'LEAVE'>();
  for (const holiday of holidays) {
    const current = hatchDates.get(holiday.holiday_date);
    hatchDates.set(holiday.holiday_date, current && current !== holiday.country_code ? 'BOTH' : holiday.country_code);
  }
  for (const override of overrides) {
    if (override.override_type !== 'WORK') hatchDates.set(override.work_date, 'LEAVE');
  }
      ...
      {tasks.map((task) => {
        ...
        return (
          <div key={task.task_id} className="flex min-h-[76px] border-b border-slate-100 last:border-b-0">
            ...
            <div className="relative flex-1 bg-[linear-gradient(to_right,#e2e8f0_1px,transparent_1px)] bg-[size:3.333%_100%]">
              {[...hatchDates.entries()].filter(([date]) => date >= minDate && date <= maxDate).map(([date, type]) => (
                <div
                  ...
                  className={`absolute inset-y-0 z-[2] ${type === 'VN' ? 'bg-[repeating-linear-gradient(45deg,rgba(14,165,233,.14)_0_2px,transparent_2px_7px)]' : type === 'KR' ? 'bg-[repeating-linear-gradient(135deg,rgba(249,115,22,.14)_0_2px,transparent_2px_7px)]' : type === 'BOTH' ? 'bg-[repeating-linear-gradient(45deg,rgba(244,63,94,.13)_0_2px,transparent_2px_5px)]' : 'bg-[repeating-linear-gradient(135deg,rgba(139,92,246,.12)_0_2px,transparent_2px_7px)]'}`}
              ...
              {task.baseline_start && task.baseline_end && <div aria-label="Baseline" className="absolute top-2 h-3 rounded border border-dashed border-slate-400 bg-slate-100/60" style={{ left: left(task.baseline_start), width: width(task.baseline_start, task.baseline_end) }} />}
              {task.official_forecast_start && task.official_forecast_end && <div aria-label="Official Forecast" className="absolute top-7 h-3 rounded border border-blue-600 bg-blue-500/70" style={{ left: left(task.official_forecast_start), width: width(task.official_forecast_start, task.official_forecast_end) }} />}
              {task.official_forecast_start && Number(task.current_progress || 0) > 0 && <div data-testid={`shadow-actual-fill-${task.task_id}`} aria-label="Actual Progress" className="absolute top-7 z-10 h-3 rounded-l bg-blue-800" style={{ left: left(task.official_forecast_start), width: progressWidth(task.official_forecast_start, task.official_forecast_end, Number(task.current_progress || 0)) }} />}
              {task.shadow_start && task.shadow_end && <div aria-label="Shadow Candidate" className={`absolute top-12 h-3 rounded border-2 border-dashed ${delayed ? 'border-orange-500 bg-orange-200/70' : 'border-teal-500 bg-teal-200/70'} ${Number(task.approval_required) ? 'opacity-60' : ''}`} style={{ left: left(task.shadow_start), width: width(task.shadow_start, task.shadow_end) }} />}
            </div>
          </div>
        );
      })}
      ...
      <main className="mx-auto max-w-[1700px] space-y-4 p-4 md:p-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><h2 className="flex items-center gap-2 text-sm font-black"><Play className="h-4 w-4 text-orange-500" />{lang === 'vi' ? 'Tính toán Shadow' : 'Shadow 재산정'}</h2><p className="mt-1 text-[10px] text-slate-500">Engine {shadow?.run?.engine_version || '3A.1.0'} · {shadow?.run?.input_fingerprint?.slice(0, 16) || 'no run'}</p></div>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <ShadowGantt tasks={impactedTasks} lang={lang} holidays={holidays} overrides={overrides} />
        </section>
