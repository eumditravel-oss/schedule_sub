...
export function ManagerOperationsPage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6">
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><h2 className="mb-3 font-bold">검토 대기</h2><div className="space-y-2 text-sm"><div className="flex justify-between"><span>Forecast 승인</span><b>{approvals.pending ?? 0}</b></div><div className="flex justify-between"><span>초과근무</span><b>{approvals.overtime ?? 0}</b></div><div className="flex justify-between"><span>정정 요청</span><b>{approvals.corrections ?? 0}</b></div><div className="flex justify-between"><span>Blocked</span><b>{schedule.blocked ?? 0}</b></div></div>{canManage && pending.length > 0 && <p className="mt-3 text-xs text-slate-500">승인 상세는 기존 Forecast 승인 화면에서 처리합니다.</p>}</div></div></section>
