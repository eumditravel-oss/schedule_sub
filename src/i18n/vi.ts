// src/i18n/vi.ts
import { TranslationKeys } from './ko';

export const vi: Record<TranslationKeys, string> = {
  headerTitle: 'Lịch dự án nhóm phát triển',
  activeProjectsTab: 'Dự án đang thực hiện',
  completedProjectsTab: 'Hoàn thành năm',
  completedProjectsYear: 'Hoàn thành năm {year}',
  yearOption: '{year}',
  addProject: 'Thêm dự án',
  editProject: 'Sửa dự án',
  deleteProject: 'Xóa dự án',
  completeProject: 'Đánh dấu hoàn thành',
  reopenProject: 'Chuyển lại dự án đang thực hiện',
  currentWorker: 'Người dùng hiện tại',
  selectWorker: 'Cần chọn',
  addTask: 'Thêm công việc',
  editTask: 'Sửa công việc',
  deleteTask: 'Xóa công việc',
  backToList: 'Quay lại danh sách',
  view30Days: 'Xem 30 ngày',
  viewMonth: 'Xem theo tháng',
  prev: 'Trước',
  today: 'Hôm nay',
  next: 'Sau',
  prevMonth: 'Tháng trước',
  nextMonth: 'Tháng sau',
  projectInfo: 'Thông tin dự án',
  worker: 'Người thực hiện',
  taskContent: 'Nội dung công việc',
  progress: 'Tiến độ',
  startDate: 'Ngày bắt đầu',
  endDate: 'Ngày kết thúc',
  completedDate: 'Ngày hoàn thành',
  completedBy: 'Người hoàn thành',
  participatingWorkers: 'Thành viên tham gia',
  statusNone: 'Chưa làm',
  statusInProgress: 'Đang làm',
  statusCompleted: 'Hoàn thành',
  statusIssue: 'Gặp sự cố',
  save: 'Lưu',
  cancel: 'Hủy',
  deleteConfirm: 'Bạn có chắc chắn muốn xóa?',
  noData: 'Không có dữ liệu.',
  originalText: 'Bản gốc',
  viewOriginal: 'Xem bản gốc',
  inputLanguage: 'Ngôn ngữ nhập',
  koText: 'Tiếng Hàn',
  viText: 'Tiếng Việt',
  retryTranslation: 'Thử dịch lại',
  translationFailedNotice: 'Lịch đã được lưu nhưng dịch tự động không thành công. Bạn có thể thử dịch lại sau.',
  readOnlyCompletedNotice: 'Dự án đã hoàn thành chỉ có thể xem. Hãy chuyển lại dự án đang thực hiện để chỉnh sửa.',
  workerListFixedError: 'Danh sách nhân sự chỉ sử dụng các thành viên đã được chỉ định.',
  invalidEditorError: 'Chỉ thành viên nhóm phát triển được chỉ định mới có thể chỉnh sửa.',
  completeConfirmMsg: 'Bạn có muốn đánh dấu dự án này là đã hoàn thành không? Dự án sẽ được chuyển sang danh sách hoàn thành và tiến độ sẽ là 100%.',
  reopenConfirmMsg: 'Bạn có muốn chuyển dự án này trở lại danh sách đang thực hiện không?',
  selectWorkerPrompt: 'Vui lòng chọn người thực hiện cho lịch trình này.',
  selectWorkerTitle: 'Chọn người thực hiện',
  readOnlyMode: 'Chế độ chỉ xem',
  actions: 'Thao tác',
  detailView: 'Xem chi tiết',
  yearSelect: 'Chọn năm',

  // UI Hardcoded Strings
  loading: 'Đang tải...',
  saving: 'Đang lưu...',
  translating: 'Đang dịch...',
  translationPending: 'Chờ dịch...',
  translationCompleted: 'Dịch tự động hoàn tất',
  manualTranslation: 'Sửa thủ công',
  translationFailed: 'Dịch tự động không thành công',
  projectSaveFailed: 'Đã xảy ra lỗi khi lưu dự án.',
  taskSaveFailed: 'Đã xảy ra lỗi khi lưu công việc.',
  completeProjectFailed: 'Đã xảy ra lỗi khi hoàn thành dự án.',
  reopenProjectFailed: 'Đã xảy ra lỗi khi khôi phục dự án.',
  translatedTextLabel: 'Bản dịch',
  automaticTranslationPlaceholder: 'Bản dịch tự động (có thể tự chỉnh sửa)',
  originalTag: 'Bản gốc',

  // Mobile Translation Keys
  summaryView: 'Tóm tắt',
  week7View: '7 ngày',
  gantt30View: '30 ngày',
  selectStatusTitle: 'Thay đổi trạng thái',
  mobileMenuTitle: 'Chọn menu',
  noActiveProjects: 'Không có dự án đang thực hiện.',
  noCompletedProjects: 'Không có dự án hoàn thành trong năm đã chọn.',
  noTasks: 'Không có công việc nào.',
  todayMarker: 'Hôm nay',
  tasksCount: '{count} công việc',
  prevWeek: 'Tuần trước',
  nextWeek: 'Tuần sau',
  close: 'Đóng',

  // Calendar & Holiday Translation Keys
  manageHolidays: 'Quản lý ngày nghỉ',
  leaveTypePersonal: 'Nghỉ phép',
  leaveTypeManualOff: 'Ngày nghỉ thủ công',
  leaveTypeWorkOverride: 'Chỉ định ngày làm việc',
  weeklyOff: 'Ngày nghỉ hàng tuần',
  publicHoliday: 'Ngày lễ',
  workday: 'Ngày làm việc',
  offDay: 'Ngày nghỉ',
  syncHolidays: 'Đồng bộ ngày lễ',
  syncSuccess: 'Đồng bộ hoàn tất',
  syncFailed: 'Đồng bộ không thành công',
  leaveSchedule: 'Lịch nghỉ phép',
  holidayName: 'Tên ngày nghỉ',
  period: 'Thời gian',
  note: 'Ghi chú',
  holidaySyncFailedNotice: 'Không thể đồng bộ ngày lễ tự động. Hệ thống đang sử dụng dữ liệu đã lưu hoặc dữ liệu nhập thủ công.',
  scopeType: 'Phân loại',
  selectScope: 'Chọn phân loại',
  thisWeekOffCount: 'Nghỉ {count} ngày tuần này',
  leaveSummaryFormat: 'Nghỉ phép {start} ~ {end}',
};
