...
export async function getManagerOperations(db: any, actorContext: ActorContextServer, requestedDate?: string) {
  ...
  const visibleWorklogApprovals = (worklogApprovals.results || []).filter((row: any) => !visibleIds || visibleIds.has(String(row.employee_id))).map((row: any) => ({ ...row, triage_classification: deriveWorklogTriage(row).classification, triage_reason_codes: deriveWorklogTriage(row).reasonCodes }));
  const countWorklog = (status: string) => visibleWorklogApprovals.filter((row: any) => row.approval_status === status).length;
  const countTriage = (classification: string) => visibleWorklogApprovals.filter((row: any) => row.approval_status === 'PENDING' && row.triage_classification === classification).length;
  const pendingAgingMinutes = (row: any) => row.eod_submitted_at_utc ? Math.max(0, (Date.now() - Date.parse(row.eod_submitted_at_utc)) / 60000) : 0;
  const countAging = (minutes: number) => visibleWorklogApprovals.filter((row: any) => row.approval_status === 'PENDING' && pendingAgingMinutes(row) >= minutes).length;
  return { scope: { actor_employee_id: actor.worker.id, office: actor.worker.country_code, can_manage: actor.canManage, read_only: !actor.canManage }, local_date: date, employees, worklogSummary: { employee_count: employees.length, morning_complete: count('morning','COMPLETE'), morning_late: count('morning','LATE'), morning_missing: count('morning','MISSING'), eod_complete: count('eod','COMPLETE'), eod_missing: count('eod','MISSING'), eod_submitted: visibleWorklogApprovals.length, eod_pending_approval: countWorklog('PENDING'), eod_approved: countWorklog('APPROVED'), eod_returned: countWorklog('RETURNED'), eod_rejected: countWorklog('REJECTED'), approval_normal: countTriage('NORMAL'), approval_review_required: countTriage('REVIEW_REQUIRED'), approval_exception: countTriage('EXCEPTION'), approval_aging_3h: countAging(180), approval_aging_1d: countAging(1440) }, scheduleSummary: { advanced: employees.filter((e: any) => e.schedule_variance_workdays < 0).length, delayed: employees.filter((e: any) => e.schedule_variance_workdays > 0).length, blocked: employees.filter((e: any) => e.shadow_status === 'BLOCKED').length }, approvalSummary: { pending: (approvals.results || []).length, worklog_pending: countWorklog('PENDING'), worklog_approved: countWorklog('APPROVED'), worklog_returned: countWorklog('RETURNED'), worklog_rejected: countWorklog('REJECTED'), worklog_normal: countTriage('NORMAL'), worklog_review_required: countTriage('REVIEW_REQUIRED'), worklog_exception: countTriage('EXCEPTION'), worklog_aging_3h: countAging(180), worklog_aging_1d: countAging(1440), overtime: (overtime.results || []).length, corrections: (corrections.results || []).length }, notifications: { unread: Number(unread?.count || 0) }, approvals: approvals.results || [], worklogApprovals: visibleWorklogApprovals, overtime: overtime.results || [], corrections: corrections.results || [] };
}
