...
export async function getScheduleComparison(db: any, options: ComparisonOptions) {
  ...
  const freshShadow = latestShadow?.status === 'CURRENT'
    && (!latestShadow.apply_status || latestShadow.apply_status === 'NOT_APPLIED')
    && (!latestShadow.based_on_forecast_version_id || latestShadow.based_on_forecast_version_id === officialVersionId)
    ? latestShadow : null;
  const shadowCandidateStatus = latestShadow
    ? (freshShadow ? latestShadow.status : latestShadow.status === 'CURRENT' ? 'STALE' : latestShadow.status)
    : 'NONE';
  for (const row of taskRows) {
    const shadow = shadowMap.get(row.task_id) || null;
    row.shadow = freshShadow && shadow ? { start: shadow.shadow_start, end: shadow.shadow_end, status: shadowCandidateStatus, data_confidence: shadow.data_confidence, approval_required: Number(shadow.approval_required || 0), constraint_result: shadow.constraint_result, delta_end_workdays: Number(shadow.delta_end_workdays || 0) } : { start: null, end: null, status: shadowCandidateStatus, data_confidence: latestShadow?.data_confidence || null, approval_required: 0, constraint_result: null, delta_end_workdays: 0 };
  ...
  return {
    ...
    shadow: { status: shadowCandidateStatus, fresh: Boolean(freshShadow), shadow_version_id: latestShadow?.shadow_version_id || null, run_id: latestShadow?.run_id || null, end: shadowEnd, approval_classification: freshShadow?.approval_classification || null, data_confidence: freshShadow?.data_confidence || null, stale_warning: freshShadow ? null : latestShadow ? 'SHADOW_STALE' : null },
