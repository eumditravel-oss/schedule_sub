// worker/services/capacityConflictServer.ts
...
export function detectWorkerCapacityConflictsServer(
  allProjects: any[],
  allTasks: any[],
  workers: any[] = [],
  holidays: any[] = [],
  ...
  const workerDailyAlloc = new Map<string, { worker: any; dateStr: string; entries: CapacityConflictTaskEntry[] }>();
  ...
  for (const t of scheduledTasks) {
    ...
    for (const a of assignees) {
      ...
      while (cur <= endObj) {
        ...
        const st = resolveWorkDayStatusServer(dStr, w, holidays, overrides);
        ...
        if (st.is_working_day) {
          ...
          // Avoid duplicate task entry if task is listed multiple times
  ...
  // Filter excess days (> 100% total allocation)
  interface ExcessDay {
    worker: any;
    dateStr: string;
    totalAlloc: number;
  ...
  // Group contiguous days with exact same worker and task set
  excessDays.sort((a, b) => {
  ...
  // Filter groups by project if filterProjectId is provided
